...
namespace sfanimationlibrary
{
    ...
    public class SfBlockAxle : BlockMPBase
    {
        ...
        public override bool TryPlaceBlock(IWorldAccessor world, IPlayer byPlayer, ItemStack itemstack, BlockSelection blockSel, ref string failureCode)
        {
            ...
            foreach (BlockFacing face in BlockFacing.ALLFACES)
            {
                ...
                if (block != null)
                {
                    ...
                    {
                        string locString = itemstack.Block.Code.Domain + ":" + itemstack.Block.FirstCodePart() + "-" + faceOpposite.Code[0] + face.Code[0];
                        locString = locString.Replace("-ew", "-we");
                        AssetLocation loc = new AssetLocation(locString);
                        Block toPlaceBlock = world.GetBlock(loc);
                        if (toPlaceBlock == null)
                        {
                            locString = itemstack.Block.Code.Domain + ":" + itemstack.Block.FirstCodePart() + "-" + face.Code[0] + faceOpposite.Code[0];
                            locString = locString.Replace("-ew", "-we");
                            loc = new AssetLocation(locString);
        ...
        {
            SfBEBBlockMPAxle bempaxle = world.BlockAccessor.GetBlockEntity(pos)?.GetBehavior<SfBEBBlockMPAxle>();
            if (bempaxle != null && !BEBehaviorMPAxle.IsAttachedToBlock(world.BlockAccessor, bempaxle.Block, pos))
            {
                ...
                foreach (BlockFacing face in BlockFacing.ALLFACES)
                {
                    ...
                    if (block != null && block.HasMechPowerConnectorAt(world, pos, face.Opposite) && world.BlockAccessor.GetBlockEntity(pos)?.GetBehavior<SfBEBBlockMPAxle>()?.disconnected == false) connected = true;
