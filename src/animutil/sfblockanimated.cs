...
{
    //currently disabled and not used for anything - the bbanimatedtoggle does the same thing
    public class SfBlockAnimated : BlockDisplayCase
    {
        public override WorldInteraction[] GetPlacedBlockInteractionHelp(IWorldAccessor world, BlockSelection selection, IPlayer forPlayer)
        {
            var be = world.BlockAccessor.GetBlockEntity(selection.Position) as SfBEBlockAnimated;
        ...
        {
            var be = GetBlockEntity<SfBEBlockAnimated>(blockSel);
        ...
        public override string GetPlacedBlockInfo(IWorldAccessor world, BlockPos pos, IPlayer forPlayer)
        {
            ...
            if (switchEnabled)
            {
                bool blockOn = false;
                var state = "off";
                if (world.BlockAccessor.GetBlockEntity(pos) is SfBEBlockAnimated be)
                {
                    blockOn = be.BlockOn;
                }
                if (blockOn)
